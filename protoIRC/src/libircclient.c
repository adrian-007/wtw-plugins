...
int irc_run (irc_session_t * session)
{
	...
	while ( irc_is_connected(session) )
	{
		...
		libirc_add_to_set (session->sock, /*0, adrian_007 edit*/&err_set,/*end*/ &maxfd);

		if ( select (maxfd + 1, &in_set, &out_set, &err_set, &tv) < 0 )
		{
		...
		// adrian_007 edit
		if(FD_ISSET(session->sock, &err_set)) {
			return 1;
		}
		// end
		...
		else if ( !strcmp (command, "PRIVMSG") )
		{
			...
			{ 
				...
				if ( params[1][0] == 0x01 && params[1][msglen-1] == 0x01 )
				{
					...
					{
						// adrian_007 edit: leave first param, put buffer to second
...
int irc_cmd_part (irc_session_t * session, const char * channel, const char* reason)
{
	if ( !channel )
	{
		session->lasterror = LIBIRC_ERR_STATE;
