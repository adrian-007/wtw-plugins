...
WTWPLUGINFO plugInfo = {
	...
	L"pexTNCounter",															// nazwa wtyczki
	L"Wy�wietlanie ilo�ci wpisanych znak�w przez drug� stron�",					// opis wtyczki
	__COPYRIGHTS(2009),															// prawa autorskie
	__AUTHOR_NAME,																// autor
	__AUTHOR_CONTACT,															// dane do kontaktu z autorem
	__AUTHOR_WEBSITE,															// strona www autora
	__AUTOUPDATE_URL(L"pexTNCounter.xml"),										// url do pliku xml z danymi do autoupdate
	PLUGIN_API_VERSION,															// wersja api
	MAKE_QWORD(0, 1, 1, 2),														// wersja wtyczki
	WTW_CLASS_PROTO_EXT,														// klasa wtyczki
	0,																			// f-cja wywolana podczas klikniecia "o wtyczce"
	L"{E177D334-9D7D-4119-AB5C-82C123AEA3B3}",									// guid (jezeli chcemy tylko jedna instancje wtyczki)
	0,																			// zaleznosci
...
int __stdcall pluginLoad(DWORD callReason, WTWFUNCTIONS* fC) {
    wtw = fC;
	pexTNCounter::create(new pexTNCounter(ghInstance));
    return 0;
}

int __stdcall pluginUnload(DWORD callReason) {
	pexTNCounter::destroy();
	return 0;
}
