...
WTWPLUGINFO plugInfo = {
	...
	L"wtwClipboard",									// nazwa wtyczki
	L"Schowek WTW",										// opis wtyczki
	__COPYRIGHTS(2009),									// prawa autorskie
	__AUTHOR_NAME,										// autor
	__AUTHOR_CONTACT,									// dane do kontaktu z autorem
	__AUTHOR_WEBSITE,									// strona www autora
	__AUTOUPDATE_URL(L"wtwClipboard.xml"),				// url do pliku xml z danymi do autoupdate
	PLUGIN_API_VERSION,									// wersja api
	MAKE_QWORD(0, 1, 1, 3),								// wersja wtyczki
	WTW_CLASS_UTILITY,									// klasa wtyczki
	0,													// f-cja wywolana podczas klikniecia "o wtyczce"
	L"{BD529ECB-BE78-4482-9D72-9056270128D6}",			// guid (jezeli chcemy tylko jedna instancje wtyczki)
	0,													// zaleznosci
...
int __stdcall pluginLoad(DWORD callReason, WTWFUNCTIONS* fC) {
	...
	wtwClipboard::create(new wtwClipboard(ghInstance));
	
    return 0;
}

int __stdcall pluginUnload(DWORD callReason) {
	wtwClipboard::destroy();
