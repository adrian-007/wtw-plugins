...
WTWPLUGINFO plugInfo = {
	...
	L"wtwStatusHistory",									// nazwa wtyczki
	L"Czyli historia naszych ostatnich opis�w",				// opis wtyczki
	__COPYRIGHTS(2009),										// prawa autorskie
	__AUTHOR_NAME,											// autor
	__AUTHOR_CONTACT,										// dane do kontaktu z autorem
	__AUTHOR_WEBSITE,										// strona www autora
	__AUTOUPDATE_URL(L"wtwStatusHistory.xml"),				// url do pliku xml z danymi do autoupdate
	PLUGIN_API_VERSION,										// wersja api
	MAKE_QWORD(0, 2, 1, 3),									// wersja wtyczki
	WTW_CLASS_UTILITY,										// klasa wtyczki
	0,														// f-cja wywolana podczas klikniecia "o wtyczce"
	L"{1A2FD88B-FB2B-4DC0-896E-7A328FBFEE71}",				// guid (jezeli chcemy tylko jedna instancje wtyczki)
	0,														// zaleznosci
...
int __stdcall pluginLoad(DWORD /*callReason*/, WTWFUNCTIONS* fC) {
	...
	wtw_t::preloadIcon(wtw, ghInstance, L"wtwStatusHistory/icon16", L"wtwStatusIcon16.png", IDB_PNG1);
	wtw_t::preloadIcon(wtw, ghInstance, L"wtwStatusHistory/icon32", L"wtwStatusIcon32.png", IDB_PNG2);

	wtwStatusHistoryManager::create(new wtwStatusHistoryManager(ghInstance));
	...
	// dodawanie strony do opcji
	{
		...
		def.id			= L"wtwStatusHistory/Options";
		def.caption		= L"Historia status�w";
		def.parentId	= WTW_OPTIONS_GROUP_PLUGINS;
		def.iconId		= L"wtwStatusHistory/icon16";
...
int __stdcall pluginUnload(DWORD /*callReason*/) {
	...
	wtwStatusHistoryManager::destroy();
