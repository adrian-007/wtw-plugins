...
WTWPLUGINFO plugInfo = {
	...
	L"wtwFTPsender",									// nazwa wtyczki
	L"Wysy�anie plik�w na serwer FTP",					// opis wtyczki
	__COPYRIGHTS(2009),									// prawa autorskie
	__AUTHOR_NAME,										// autor
	__AUTHOR_CONTACT,									// dane do kontaktu z autorem
	__AUTHOR_WEBSITE,									// strona www autora
	__AUTOUPDATE_URL(L"wtwFTPsender.xml"),				// url do pliku xml z danymi do autoupdate				
	PLUGIN_API_VERSION,									// wersja api
	MAKE_QWORD(0, 4, 2, 1),								// wersja wtyczki
	WTW_CLASS_UTILITY,									// klasa wtyczki
	0,													// f-cja wywolana podczas klikniecia "o wtyczce"
	L"{4813DBB2-8A8E-4214-8128-2626B0C136C3}",			// guid (jezeli chcemy tylko jedna instancje wtyczki)
	0,													// zaleznosci
...
int __stdcall pluginLoad(DWORD callReason, WTWFUNCTIONS* fC) {
	...
	wtwFTPsender::create(new wtwFTPsender(ghInstance));
	...
	// dodawanie strony do opcji
	{
		...
		def.id			= L"wtwFTPsender/Options";
		def.caption		= L"Klient FTP";
...
int __stdcall pluginUnload(DWORD callReason) {
	...
	wtw->fnCall(WTW_CCB_FUNCT_CLEAR, (WTW_PARAM)ghInstance, 0);

	wtwFTPsender::destroy();
